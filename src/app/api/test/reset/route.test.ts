...
function createRequest(secret: string) {
  return new Request("https://volume.fitness/api/test/reset", {
    method: "POST",
    headers: {
...
describe("POST /api/test/reset", () => {
  ...
  it("returns 404 in production deployments", async () => {
    ...
    const response = await POST(createRequest("test-secret") as any);
  ...
  it("rejects invalid secrets in preview deployments", async () => {
    ...
    const { POST } = await import("./route");
    const response = await POST(createRequest("wrong-secret") as any);
